...
type ResourceRateField = `${ResourceKey}PerHour`;
type ResourceCapacityField = `${ResourceKey}Capacity`;
type ResourceSettlementData = Record<ResourceKey, number> & {
...
export class ResourceService {
  ...
  async settleByCityId(cityId: string) {
    ...
    const nextData: Partial<ResourceSettlementData> = {
      lastCollectedAt: now
    };
  ...
  private rateField(key: ResourceKey): ResourceRateField {
    return `${key}PerHour`;
  }
